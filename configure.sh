...
read -p "VERSION (2.3.x: please change it if the default value is not appropriate) = " VERSION;echo VERSION=$VERSION>>$CONF
...
read -p "BRANCH (toolkit/upgrade: please change the default value if necessary but please be careful so that the branch has the new toolkit) = " BRANCH;BRANCH=${BRANCH:-"toolkit/upgrade"};echo BRANCH=$BRANCH>>$CONF
