...
YUM=/tmp/yum.list;sudo yum update -y;sudo yum -y remove mysql55*;sudo yum list installed>>$YUM;for x in mysql56-server php56 phpMyAdmin;do grep -q $x $YUM||sudo yum -y install $x;done
